...
RC Table::drop(std::string file_path)
{
  RC rc = sync();//刷新所有脏页

    if(rc != RC::SUCCESS) return rc;

    std::string path = table_meta_file(file_path.c_str(), name());
    if(unlink(path.c_str()) != 0) {
        LOG_ERROR("Failed to remove meta file=%s, errno=%d", path.c_str(), errno);
        return RC::FILE_REMOVE;
    }

    std::string data_file = file_path + "/" + name() + TABLE_DATA_SUFFIX;
    if(unlink(data_file.c_str()) != 0) { // 删除描述表元数据的文件
        LOG_ERROR("Failed to remove data file=%s, errno=%d", data_file.c_str(), errno);
        return RC::FILE_REMOVE;
    }

    // std::string text_data_file = file_path + "/" + name() + TABLE_TEXT_DATA_SUFFIX;
    ...
    for (int i = 0; i < index_num; i++) {  // 清理所有的索引相关文件数据与索引元数据
        ...
        std::string index_file = table_index_file(file_path.c_str(), name(), index_meta->name());
